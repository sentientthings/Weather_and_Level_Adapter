...
float DS18B20::getCRCTempC(void)
{
  ScratchPad scratchPad;
  uint8_t retries = 7;
  bool validCRC = false;
    ...
    }while (!validCRC && retries > 0);


  /*int16_t rawTemperature = (((int16_t)scratchPad[TEMP_MSB]) << 8) | scratchPad[TEMP_LSB];

  float temp = 0.0625 * rawTemperature;
  if ((temp < -55) || !validCRC) return DEVICE_DISCONNECTED;
  return temp;*/
  if (validCRC)
  {
   int16_t rawTemperature = (((int16_t)scratchPad[TEMP_MSB]) << 8) | scratchPad[TEMP_LSB];
...
int16_t DS18B20::getCRCTempRAW(void)
  {
    ScratchPad scratchPad;
    uint8_t retries = 7;
    bool validCRC = false;
...
uint8_t DS18B20::getCRC(void)
{
  ScratchPad crcScratch;
  readScratchPad(crcScratch, 9);

  return crcScratch[8];
}

uint8_t DS18B20::calCRC(void)
{
  uint8_t len = 8;
  uint8_t n = 0;
  ScratchPad crcScratch;
  readScratchPad(crcScratch, len);
...
//Serial.println();
 ...
 //return _wire->crc8((uint8_t) myPointer, 9);

 //return _wire->crc8(uint8_t *crcScratch, 9);
