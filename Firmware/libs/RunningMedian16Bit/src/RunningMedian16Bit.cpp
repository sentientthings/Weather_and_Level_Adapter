...
float RunningMedian::getAverage()
{
  ...
  float sum = 0;
  for (uint16_t i=0; i< _cnt; i++) sum += _ar[i];
  return sum / _cnt;
}
...
float RunningMedian::getAverage(uint16_t nMedians)
{
  ...
  uint16_t start = ((_cnt - nMedians) / 2);
  uint16_t stop = start + nMedians;

  if (_sorted == false) sort();

  float sum = 0;
  for (uint16_t i = start; i < stop; i++) sum += _ar[_p[i]];
  return sum / nMedians;
}

uint16_t RunningMedian::getElement(const uint16_t n)
{
...
uint16_t RunningMedian::getSortedElement(const uint16_t n)
{
  if ((_cnt == 0) || (n >= _cnt)) return NAN;
